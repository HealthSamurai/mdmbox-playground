...
Backend URLs and credentials are read from `.env` (see
[.env.example](.env.example)); its defaults match the shared docker-compose
stack. Without a `.env` the dev proxy sends no `Authorization` header and
FHIR requests fail with `401`. Set `MDMBOX_AUTH` when MDMbox runs with
